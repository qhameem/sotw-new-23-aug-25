...
/**
 * Demo of text rewriting techniques for the Pokee product description
...
class TextRewritingDemo
{
    ...
    public function extractiveSummarize(string $text, int $numSentences = 2): string
    {
        ...
        foreach ($sentences as $sentence) {
            ...
            $score += (substr_count(strtolower($sentence), 'research') * 3);
            $score += (substr_count(strtolower($sentence), 'communication') * 3);
            $score += (substr_count(strtolower($sentence), 'admin') * 3);
    ...
    public function templateRewrite(string $productName, string $originalDescription): string
    {
        $templates = [
            ...
            "{$productName} solves your admin, research and communication tasks across all your apps.",
            "{$productName} automates the work you wish someone else could do.",
            "Now you can delegate tasks to {$productName}, your virtual assistant for research and communication."
        ];

        // Pick a random template or one that best matches keywords in the original description
        $keywords = ['powerful', 'easy', 'best', 'solution', 'tool', 'professional', 'innovative', 'admin', 'research', 'communication', 'tasks'];
        $keywordCounts = [];
    ...
    public function ruleBasedRewrite(string $originalText): string
    {
        ...
        $replacements = [
            ...
            '/(admin|research|communication) tasks/i' => 'administrative, research and communication workflows',
            '/(sales leads|meeting\'s action items|customer service tickets)/i' => 'business processes',
            '/wish someone else could do/i' => 'want to automate',
            '/Instruct like an assistant/i' => 'Direct like a virtual assistant',
        ];
    ...
    public function generateTaglineVariations(string $productName, string $originalTagline, int $limit = 3): array
    {
        ...
        $templates = [
            ...
            "{$productName}: Automating {$words[array_rand($words)]} tasks",
            "{$productName}: Your {$words[array_rand($words)]} assistant",
        ];
...
// Sample product data for Pokee
$productName = "Pokee";
$originalDescription = "Pokee solves your admin, research and communication tasks across all your apps
From compiling data on new sales leads, to summarizing every meeting's action items and triaging customer service tickets, everyone has work that they wish someone else could do. Now that's possible with Pokee.

Instruct Pokee like an assistant and iterate together until the plan is right. Then, at your direction, Pokee will get to work, researching, creating and translating information across the internet and your apps to efficiently tick of your tasks, one by one.";
$originalTagline = "Solve admin, research and communication tasks";
