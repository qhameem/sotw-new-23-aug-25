<template>
  <div class="wysiwyg-editor bg-white border border-gray-300 rounded-md focus-within:ring-1 focus-within:ring-sky-400 focus-within:border-sky-400 flex flex-col">
    <div v-if="editor" class="flex items-center p-1 border-b border-gray-300 gap-1 bg-gray-50">
      <button type="button" @click="editor.chain().focus().undo().run()" :disabled="!editor.can().undo()" class="p-1 rounded hover:bg-gray-200 disabled:opacity-50">
      ...
      <button type="button" @click="editor.chain().focus().redo().run()" :disabled="!editor.can().redo()" class="p-1 rounded hover:bg-gray-200 disabled:opacity-50">
      ...
      <button type="button" @click="editor.chain().focus().toggleBold().run()" :class="{ 'is-active': editor.isActive('bold') }" class="p-1 rounded hover:bg-gray-200">
      ...
      <button type="button" @click="editor.chain().focus().toggleItalic().run()" :class="{ 'is-active': editor.isActive('italic') }" class="p-1 rounded hover:bg-gray-200">
      ...
      <button type="button" @click="editor.chain().focus().toggleHeading({ level: 2 }).run()" :class="{ 'is-active': editor.isActive('heading', { level: 2 }) }" class="p-1 rounded hover:bg-gray-200 font-bold text-xs">
        H2
      </button>
      <button type="button" @click="editor.chain().focus().toggleHeading({ level: 3 }).run()" :class="{ 'is-active': editor.isActive('heading', { level: 3 }) }" class="p-1 rounded hover:bg-gray-200 font-bold text-xs">
        H3
      </button>
      <button type="button" @click="editor.chain().focus().toggleBulletList().run()" :class="{ 'is-active': editor.isActive('bulletList') }" class="p-1 rounded hover:bg-gray-200">
        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="8" y1="6" x2="21" y2="6"></line><line x1="8" y1="12" x2="21" y2="12"></line><line x1="8" y1="18" x2="21" y2="18"></line><line x1="3" y1="6" x2="3.01" y2="6"></line><line x1="3" y1="12" x2="3.01" y2="12"></line><line x1="3" y1="18" x2="3.01" y2="18"></line></svg>
      </button>
      <button type="button" @click="editor.chain().focus().toggleOrderedList().run()" :class="{ 'is-active': editor.isActive('orderedList') }" class="p-1 rounded hover:bg-gray-200">
...
const editor = useEditor({
  ...
  onUpdate: ({ editor }) => {
    ...
    // Only truncate content if maxLength is specified
  ...
  editorProps: {
    ...
    handlePaste: (view, event, slice) => {
      // Only check length limit if maxLength is specified
      if (props.maxLength !== null && props.maxLength !== undefined) {
