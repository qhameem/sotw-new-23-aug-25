@if(isset($ad) && $ad)
    <div class="ad-listing-item {{ $outerClass ?? 'mb-4 mt-2' }} rounded-lg border border-gray-200 border-l-4 bg-white p-4 transition-colors hover:bg-gray-50" style="border-left-color: var(--color-primary-500);">
        ...
        @if($ad->type === 'image_banner')
            <a href="{{ route('ads.click', ['ad' => $ad, 'zone' => $zoneSlug ?? $ad->adZones->first()?->slug]) }}" @if($ad->open_in_new_tab) target="_blank" rel="{{ \App\Support\OutboundLink::rel($ad->target_url, 'ad') }}" @endif class="group relative block" aria-label="Open {{ $ad->internal_name }} website">
        ...
        @elseif($ad->type === 'product_listing_card')
            <a href="{{ route('ads.click', ['ad' => $ad, 'zone' => $zoneSlug ?? $ad->adZones->first()?->slug]) }}" @if($ad->open_in_new_tab) target="_blank" rel="{{ \App\Support\OutboundLink::rel($ad->target_url, 'ad') }}" @endif class="group flex items-center gap-3 rounded-lg p-1" aria-label="Open {{ $ad->internal_name }} website">
        ...
        @elseif($ad->type === 'text_link')
            <a href="{{ route('ads.click', ['ad' => $ad, 'zone' => $zoneSlug ?? $ad->adZones->first()?->slug]) }}" @if($ad->open_in_new_tab) target="_blank" rel="{{ \App\Support\OutboundLink::rel($ad->target_url, 'ad') }}" @endif class="inline-flex items-center gap-1 text-blue-600 hover:underline " aria-label="Open {{ $ad->internal_name }} website">
