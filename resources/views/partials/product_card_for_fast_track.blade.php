...
<article class="p-4 flex items-center gap-3 md:gap-3 transition relative group cursor-pointer hover:bg-gray-50 border-b"
    ...
    <div class="flex-1">
        ...
        <p class="text-gray-800 text-base mt-0.5 line-clamp-2">{{ $product->tagline }}</p>

        <div class="mt-2">
            <x-scheduled-datepicker name="publish_date[{{ $product->id }}]" value="{{ now()->toDateString() }}"
                class="product-date" />
        </div>
    </div>
    ...
    <div class="flex flex-col items-center justify-center ml-1 md:ml-2 w-24 flex-shrink-0">
        <input type="checkbox"
            class="product-checkbox h-5 w-5 text-primary-500 border-gray-300 rounded focus:ring-primary-500"
            value="{{ $product->id }}" data-price="30">
