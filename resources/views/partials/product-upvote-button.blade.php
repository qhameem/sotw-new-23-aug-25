<div>
    @if (!$product->is_promoted)
        @php
            ...
            $preferMomentumLabels = $preferMomentumLabels ?? false;
        @endphp
        ...
        @else
            <div x-data="upvote(
                ...
                {{ Auth::check() ? 'true' : 'false' }},
                '{{ csrf_token() }}',
                {{ (int) ($product->outbound_clicks_count ?? 0) }},
                {{ (int) ($product->impressions ?? 0) }},
                {{ $preferMomentumLabels ? 'true' : 'false' }}
            )" class="">

                <button type="button" @click.stop="toggleUpvote"
                    @class([
                        'group flex flex-col items-center justify-center rounded-xl border border-slate-200 transition-colors duration-200 hover:border-slate-300',
                        'h-[72px] w-[76px] px-2' => $preferMomentumLabels,
                        'h-16 w-16' => !$preferMomentumLabels,
                    ])>
                    ...
                    <span x-text="displayCount()" class="mt-2 text-xs font-semibold leading-none tracking-tight transition-colors duration-200"
                        :class="isUpvoted ? 'text-rose-600' : (showNumericVotes() ? 'text-slate-800' : 'text-slate-500')"></span>
                    <span x-show="displaySubtext()"
                        x-text="displaySubtext()"
                        class="mt-1 text-[10px] leading-none text-slate-400"></span>
                </button>
                <p x-show="errorMessage" x-text="errorMessage" class="mt-1 text-xs text-red-500"></p>
            </div>
        @endif
