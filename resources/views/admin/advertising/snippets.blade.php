    ...
    <form action="{{ route('admin.code-snippets.store') }}" method="POST">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <select name="page" id="page" class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md">
                    <option value="all">All Pages</option>
                    <option value="home">Home</option>
                    <option value="products.index">Products</option>
                    <option value="articles.index">Articles</option>
                </select>
            </div>
            <div>
        ...
        <div class="mt-6">
            ...
            <textarea name="code" id="code" rows="5" class="mt-1 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md"></textarea>
        </div>
        <div class="mt-6">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                @foreach ($snippets as $snippet)
                    <tr>
                        ...
                        <td class="px-6 py-4">
                            <pre class="bg-gray-100 p-2 rounded-md text-sm text-gray-800"><code>{{ e($snippet->code) }}</code></pre>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <form action="{{ route('admin.code-snippets.destroy', $snippet->id) }}" method="POST" onsubmit="return confirm('Are you sure?');">
                                @csrf
                                @method('DELETE')
