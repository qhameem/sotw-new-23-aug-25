    ...
    <form action="{{ route('admin.advertising.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="bg-white shadow-md rounded-lg p-6">
            <div class="mb-4">
                ...
                <input type="text" name="internal_name" id="internal_name" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required>
            </div>

            <div class="mb-4">
                <label for="tagline" class="block text-gray-700 text-sm font-bold mb-2">Tagline:</label>
                <input type="text" name="tagline" id="tagline" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required>
            </div>

            <div class="mb-4">
                <label for="target_url" class="block text-gray-700 text-sm font-bold mb-2">Target URL:</label>
                <input type="url" name="target_url" id="target_url" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required>
            </div>
            ...
            <div class="mb-4">
                ...
                <img id="logo_preview" src="" alt="Logo Preview" class="mt-2 w-20 h-20 object-cover rounded-lg" style="display: none;">
            </div>

            <div class="mb-4">
                <label for="product_id" class="block text-gray-700 text-sm font-bold mb-2">Or Select Existing Product:</label>
                <select name="product_id" id="product_id" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                    ...
                    @foreach(\App\Models\Product::latest()->get() as $product)
                        <option value="{{ $product->id }}" data-tagline="{{ $product->tagline }}" data-url="{{ $product->link }}" data-logo="{{ $product->logo_url }}">{{ $product->name }}</option>
                    @endforeach
                </select>
            </div>

            <input type="hidden" name="ad_zone_id" value="{{ \App\Models\AdZone::where('slug', 'sponsors')->first()?->id }}">
