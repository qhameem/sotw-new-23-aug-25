...
@if($pendingCustomCategoryCount > 0)
    <div id="custom-category-modal-{{ $product->id }}" class="fixed inset-0 z-[9999] hidden items-start justify-center overflow-hidden bg-slate-900/60 p-4" role="dialog" aria-modal="true" aria-labelledby="custom-category-title-{{ $product->id }}">
        <div class="flex max-h-[calc(100dvh-2rem)] w-full max-w-2xl flex-col overflow-hidden rounded-xl bg-white shadow-xl">
            <div class="flex flex-none items-start justify-between gap-4 border-b border-slate-200 bg-white px-6 py-5">
                <div>
            ...
            <div class="flex flex-none items-center justify-end gap-3 border-t border-slate-200 bg-white px-6 py-4">
