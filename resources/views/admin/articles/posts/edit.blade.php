<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('Edit Article Post') }} <span class="text-indigo-600 dark:text-indigo-400">- {{ $article->title }}</span>
        </h2>
    </x-slot>
                    ...
                    <form action="{{ route('admin.articles.posts.update', ['post' => $article->id]) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="mb-6">
                            ...
                            <div class="space-y-4">
                                ...
                                <div>
                                    <x-input-label for="slug" :value="__('Slug (auto-updated if title changes and slug is empty or matches old title slug)')" />
                                    <x-text-input id="slug" class="block mt-1 w-full" type="text" name="slug" :value="old('slug', $article->slug)" />
                                ...
                                <div>
                                    ...
                                    <div id="quill-editor" style="height: 300px;" class="mt-1 bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100 border border-gray-300 dark:border-gray-700 rounded-md">
                                        {!! old('content', $article->content) !!} {{-- Populate with old or existing content --}}
                                    </div>
                            ...
                            <div class="space-y-4">
                                <div>
                                    ...
                                    <select name="categories[]" id="categories" multiple class="block mt-1 w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm">
                                        @php $selectedCategories = old('categories', $article->categories->pluck('id')->toArray()); @endphp
                                ...
                                <div>
                                    ...
                                    <select name="status" id="status" class="block mt-1 w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm">
                                        @foreach($statuses as $key => $value)
                                            <option value="{{ $key }}" {{ old('status', $article->status) == $key ? 'selected' : '' }}>{{ $value }}</option>
                                        @endforeach
                                    </select>
                        ...
                        <div class="mb-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <x-text-input id="meta_title" class="block mt-1 w-full" type="text" name="meta_title" :value="old('meta_title', $article->meta_title)" />
                                    <x-input-error :messages="$errors->get('meta_title')" class="mt-2" />
                                </div>
                                <div>
                                    <x-input-label for="meta_keywords" :value="__('Meta Keywords (comma-separated)')" />
                                    <x-text-input id="meta_keywords" class="block mt-1 w-full" type="text" name="meta_keywords" :value="old('meta_keywords', $article->meta_keywords)" />
                             ...
                             <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">{{ __('(Media library/upload will be integrated here)') }}</p>
                                     @if($article->og_image)
                                        <div class="mt-2">
                                            <img src="{{ Str::startsWith($article->og_image, ['http://', 'https://']) ? $article->og_image : asset('storage/' . $article->og_image) }}" alt="OG Image Preview" class="max-h-48 rounded">
                                        </div>
                                    @endif
                                </div>
                            </div>
                        ...
                        <div class="mb-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <x-text-input id="twitter_card" class="block mt-1 w-full" type="text" name="twitter_card" :value="old('twitter_card', $article->twitter_card)" />
                                    <x-input-error :messages="$errors->get('twitter_card')" class="mt-2" />
                                </div>
                                <div>
                                    <x-input-label for="twitter_title" :value="__('Twitter: Title')" />
                                    <x-text-input id="twitter_title" class="block mt-1 w-full" type="text" name="twitter_title" :value="old('twitter_title', $article->twitter_title)" />
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            // Initial content is set by Blade in the div#quill-editor: {!! old('content', $article->content) !!}
            // And also in the hidden input: value="{{ old('content', $article->content) }}"
            // Quill automatically picks up content from its container div.

            // Slug generation
            const titleInput = document.getElementById('title');
            const slugInput = document.getElementById('slug');
            // If slug is pre-filled and different from an auto-generated version of the current title, assume it was manually set.
            // Or, more simply, if slugInput has a value, assume it was intentional.
            let manualSlugEdit = slugInput.value !== '' && slugInput.value !== generateSlug(titleInput.value);
            let debounceTimer;
            ...
            titleInput.addEventListener('input', function () {
                ...
                debounceTimer = setTimeout(function() {
                    if (!manualSlugEdit) {
                        const currentSlug = generateSlug(titleInput.value);
                        // Only update if the generated slug is different, to avoid unnecessary changes
                        if (slugInput.value !== currentSlug) {
            ...
            slugInput.addEventListener('input', function() {
                // Any manual input to slug field means user wants control
                if (slugInput.value !== generateSlug(titleInput.value)) {
                    manualSlugEdit = true;
                }
                // If user clears the slug, re-enable auto-generation from title
                if (slugInput.value === '') {
             ...
             // Initial check: if slug is empty and title is not, generate slug.
            if (titleInput.value !== '' && slugInput.value === '') {
                slugInput.value = generateSlug(titleInput.value);
                manualSlugEdit = false; // Since it was auto-generated now
            }
                ...
                if (file) {
                    ...
                    xhr.onload = function() {
                        ...
                        if (xhr.status >= 200 && xhr.status < 300) {
                            ...
                            if (response.success) {
                                imagePathInput.value = response.path; // Store the relative path from storage/app/public
                                imagePreview.src = response.url;    // Use the full public URL for preview
            ...
            // For edit form, if imagePathInput already has a value (from the model), ensure preview is shown.
            // The src for imagePreview is already set by Blade if $article->featured_image_path exists.
            // This logic is mostly handled by the Blade template's initial rendering of the imagePreview src.
        });
    </script>
    @endpush
</x-app-layout>