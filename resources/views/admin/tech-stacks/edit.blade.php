...
@section('header-title')
    Edit Tech Stack: {{ $tech_stack->name }}
@endsection
...
<div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
    <div class="bg-white shadow-sm sm:rounded-lg p-6">
        <form method="POST" action="{{ route('admin.tech-stacks.update', $tech_stack) }}">
            @csrf
            @method('PUT')
            <div class="space-y-4">
                <div>
                    <label for="name" class="block text-sm font-medium text-gray-700 mb-1">Name *</label>
                    <input type="text" name="name" id="name" value="{{ old('name', $tech_stack->name) }}" required
                ...
                <div>
                    ...
                    <input type="text" name="icon" id="icon" value="{{ old('icon', $tech_stack->icon) }}"
                           class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-primary-500 focus:border-primary-500"
