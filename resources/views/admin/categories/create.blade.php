...
<div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
    <h1 class="text-2xl font-bold pt-14 mb-4">Create Category</h1>
    @if($errors->any())
    ...
    <form action="{{ route('admin.categories.store') }}" method="POST" class="bg-white p-6 rounded shadow">
                ...
                @forelse($categoryTypes as $type)
                    <label class="flex items-center space-x-2 text-sm">
                        <input type="checkbox" name="category_types[]" value="{{ $type->id }}"
                               ...
                               @if(is_array(old('category_types')) && in_array($type->id, old('category_types'))) checked @endif>
                        <span>{{ $type->name }}</span>
                    </label>
                @empty
                    <p class="text-gray-500  col-span-full">No category types available. Please create some first.</p>
                @endforelse
            </div>
        </div>
        ...
        <button type="submit"
                ...
                style="color: var(--color-primary-button-text);">Create</button>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        if (nameInput && slugInput) {
            nameInput.addEventListener('input', function () {
                ...
                // and remove characters that are not alphanumeric or hyphens.
