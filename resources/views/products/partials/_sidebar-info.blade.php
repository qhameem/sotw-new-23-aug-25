            ...
            <div class="flex flex-wrap gap-2">
                @foreach($bestForCategories as $category)
                    <a href="{{ route('categories.show', ['category' => $category->slug]) }}"
                        class="inline-flex items-center px-2.5 py-1 text-gray-700 text-[0.65rem] font-medium rounded-full hover:bg-gray-200">
                        {{ $category->name }}
                    </a>
                @endforeach
            </div>
                    ...
                    <li class="flex items-start">
                        <a href="{{ route('products.show', $similarProduct->slug) }}">
                            @if($similarProduct->logo)
                                <img src="{{ Str::startsWith($similarProduct->logo, 'http') ? $similarProduct->logo : asset('storage/' . $similarProduct->logo) }}"
                                    alt="{{ $similarProduct->name }} logo" class="w-10 h-10 object-contain rounded-xl mr-4">
                            @elseif($similarProduct->link)
                                <img src="{{ 'https://www.google.com/s2/favicons?sz=64&domain_url=' . urlencode($similarProduct->link) }}"
                                    alt="{{ $similarProduct->name }} favicon" class="w-10 h-10 object-contain rounded-xl mr-4">
                            @endif
                        </a>
                        <div class="flex-1">
                            <a href="{{ route('products.show', $similarProduct->slug) }}"
                                class="font-medium text-sm text-gray-900 hover:text-primary-500">{{ $similarProduct->name }}</a>
