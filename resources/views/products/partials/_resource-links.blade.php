@php
    ...
    foreach ($makerLinks as $link) {
        ...
        if ($platform) {
            ...
            if (Str::contains($host, 'github.com')) {
                $displayLink = 'GitHub Repository';
            } elseif (Str::contains($host, 'docs.')) {
                $displayLink = 'Documentation';
            } elseif (Str::contains($host, 'help.')) {
        ...
        @if($xProfileUrl || !empty($socialLinks))
            <article class="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
                ...
                <div class="mt-4 space-y-3">
                    @if($xProfileUrl)
                        <a href="{{ $xProfileUrl }}" target="_blank" rel="{{ \App\Support\OutboundLink::rel($xProfileUrl, 'product_social') }}"
                            class="flex items-center gap-2 text-sm font-medium text-gray-700 underline-offset-2 hover:text-gray-900 hover:underline">
                    ...
                    @foreach($socialLinks as $socialLink)
                        <a href="{{ $socialLink['link'] }}" target="_blank" rel="{{ \App\Support\OutboundLink::rel($socialLink['link'], 'product_social') }}"
                            class="flex items-center gap-2 text-sm font-medium text-gray-700 underline-offset-2 hover:text-gray-900 hover:underline">
                            {!! $socialLink['icon'] !!}
            ...
            <article class="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
                ...
                <div class="mt-4 space-y-3">
                    @foreach($resourceLinks as $resourceLink)
                        <a href="{{ $resourceLink['link'] }}" target="_blank" rel="{{ \App\Support\OutboundLink::rel($resourceLink['link'], $resourceLink['source']) }}"
                            class="flex items-center gap-2 text-sm font-medium text-gray-700 underline-offset-2 hover:text-gray-900 hover:underline">
                            <svg class="size-3.5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
