...
<div class="mb-4">
    <label class="block mb-1 font-semibold">Product URL*</label>
    <div class="flex items-center relative">
        <input type="url" name="link" x-model="link" @input.debounce.500ms="checkUrlUnique" @blur="checkUrlUnique" @keydown.enter.prevent="checkUrlUnique" class="w-full border border-gray-300 rounded-md px-3 py-2 pr-10 text-sm focus:border-primary-500 :border-primary-500 focus:ring-primary-500 :ring-primary-500" required :disabled="isEditMode" :class="{'bg-gray-100 cursor-not-allowed': isEditMode}">
...
<div class="mb-4">
    ...
    <input type="text" name="name" x-model="name" class="w-full text-sm border border-gray-300  rounded-md px-3 py-2 focus:border-primary-500 :border-primary-500 focus:ring-primary-500 " required :disabled="isEditMode" :class="{'bg-gray-100  cursor-not-allowed': isEditMode}" @input="updateSlugOnNameChange">
</div>
<div class="mb-4">
    <label class="block mb-1 font-semibold">Tagline* (Max 150 characters)</label>
    <input type="text" name="tagline" x-model="tagline" maxlength="150" class="w-full text-sm border border-gray-300  rounded-md px-3 py-2 focus:border-primary-500  focus:ring-primary-500" required>
</div>
<div class="mb-4">
    <label class="block mb-1 font-semibold">Tagline on Product Page* (Max 150 characters)</label>
    <input type="text" name="product_page_tagline" x-model="product_page_tagline" maxlength="150" class="w-full text-sm border border-gray-300 rounded-md px-3 py-2 focus:border-primary-500 focus:ring-primary-500" required>
</div>
<div class="mb-4">
...
<div class="mb-4">
    ...
    <div x-show="!logoPreviewUrl"
        ...
        <div class="space-y-1 text-center">
            ...
            <div class="flex text-sm text-gray-600">
                <p class="pl-1">Select new logo to upload/propose</p>
            </div>
    ...
    <template x-if="logoPreviewUrl">
        <div class="mt-2 relative group">
            ...
            <button @click="removePreviewLogo()" type="button"
                    class="absolute top-0 right-0 -mt-2 -mr-2 p-1 bg-red-500 text-white rounded-full opacity-0 group-hover:opacity-100 hover:bg-red-600 transition-opacity focus:outline-none">
...
<div class="mb-4">
        ...
        <div class="flex flex-wrap gap-2" x-show="selectedCategoriesDisplay.length > 0">
            <template x-for="category in selectedCategoriesDisplay" :key="category.id">
                <span class="inline-flex items-center px-2.5 py-1 bg-gray-100 text-gray-700 text-xs font-medium rounded-full group hover:bg-gray-200">
                    ...
                    <button @click.prevent="deselectCategory(category.id)" type="button"
                            class="ml-1.5 -mr-1 flex-shrink-0 inline-flex items-center justify-center h-4 w-4 rounded-full text-gray-400 hover:text-gray-600 focus:outline-none focus:bg-gray-300"
    ...
    <div class="mb-4">
        ...
        <div class="mb-2 relative">
            <input type="text" x-model="categorySearchTerm" x-ref="categorySearchInput" placeholder="Search software categories..."
                   class="w-full text-sm text-gray-700 border-gray-300 rounded-md px-3 py-2 focus:ring-primary-500 focus:border-primary-500 pr-8">
            <button type="button" x-show="categorySearchTerm.length > 0" @click="categorySearchTerm = ''; $refs.categorySearchInput.focus()"
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-x-4 gap-y-1 max-h-60 overflow-y-auto pr-2 border rounded-md p-3">
...
<div class="flex flex-row justify-between text-xs text-gray-500 mt-6">
    <div>
