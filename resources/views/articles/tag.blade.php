<x-app-layout :metaTitle="'Posts Tagged: ' . $tag->name"
              :metaDescription="'Browse articles tagged with ' . $tag->name">
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800  leading-tight">
                {{ __('Tag Archives') }}: <span class="text-indigo-600 ">#{{ $tag->name }}</span>
            </h2>
                            ...
                            @foreach($allTags as $tag)
                                ...
                                <a href="{{ route('articles.tag', $tag->slug) }}"
                                   class="inline-block rounded-full px-3 py-1 text-xs font-semibold transition duration-150 ease-in-out
                                          @if(isset($tag) && $tag->id === $tag->id) bg-primary-500 text-white
