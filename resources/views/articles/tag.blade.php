...
@section('title', 'Posts Tagged: ' . $tag->name . ' | Software on the Web')

@section('meta_description', 'Browse articles tagged with ' . $tag->name)

@section('header-title')
    <span>Tag Archives: #{{ $tag->name }}</span>
@endsection
        ...
        <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="grid grid-cols-1 md:grid-cols-12 gap-8">
                <div class="md:col-span-9">
                    ...
                    @else
                        <div class="bg-white  shadow-lg rounded-lg p-6">
                            <p class="text-gray-700  text-center">{{ __('No articles found with this tag.') }}</p>
                        </div>
                    @endif
                </div>
                ...
                <aside class="md:col-span-3 space-y-8">
                    ...
                    <div class="bg-white  shadow-lg rounded-lg p-6">
                        <h3 class="text-lg font-semibold text-gray-900  mb-4">{{ __('All Tags') }}</h3>
                        <div class="flex flex-wrap gap-2">
                            @foreach($allTags as $archiveTag)
                                @if($archiveTag->articles_count > 0)
                                <a href="{{ route('articles.tag', $archiveTag->slug) }}"
                                   class="inline-block rounded-full px-3 py-1 text-xs font-semibold transition duration-150 ease-in-out
                                          @if($archiveTag->id === $tag->id) bg-primary-500 text-white
                                          @else bg-indigo-100 text-indigo-700   hover:bg-indigo-200  @endif">
                                    #{{ $archiveTag->name }} ({{$archiveTag->articles_count}})
                                </a>
                                @endif
                            @endforeach
                        </div>
