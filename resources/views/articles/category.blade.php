...
@section('actions')
    <form action="{{ route('articles.search') }}" method="GET" class="flex">
        <label for="category-articles-search-query" class="sr-only">{{ __('Search articles') }}</label>
        <x-text-input id="category-articles-search-query" type="search" name="query" placeholder="Search articles..." class="py-2 px-3 rounded-l-md" :value="request('query')" />
