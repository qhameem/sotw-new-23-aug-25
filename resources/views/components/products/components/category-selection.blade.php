...
<div x-data='categorySelect(@json($categories), @json($selected), @json($type))' @click.away="isCategoryDropdownOpen = false">
    <div class="flex justify-between items-center">
        ...
        <span class="text-xs text-gray-600">Select at least 1 and up to 3</span>
    </div>
    <div class="mb-4">
        <div class="mb-2 relative">
            <div class="w-full text-sm text-gray-700 border-gray-300 rounded-md p-2 placeholder-gray-400 pr-8 flex flex-wrap gap-2 items-center border" @click="isCategoryDropdownOpen = true; $refs.categorySearchInput.focus()">
                ...
                <input type="text" x-model="categorySearchTerm" x-ref="categorySearchInput" :placeholder="selectedCategories.length > 0 ? 'Click here to see more categories' : 'Search software categories...'" class="flex-grow border-none focus:ring-0 p-0 text-sm placeholder-gray-400 placeholder:text-xs focus:outline-none" @keydown.arrow-down.prevent="highlightedCategoryIndex = Math.min(highlightedCategoryIndex + 1, filteredCategories.length - 1)" @keydown.arrow-up.prevent="highlightedCategoryIndex = Math.max(highlightedCategoryIndex - 1, -1)" @keydown.enter.prevent="if (highlightedCategoryIndex > -1) { toggleCategory(filteredCategories[highlightedCategoryIndex].id); highlightedCategoryIndex = -1; }" @focus="isCategoryDropdownOpen = true">
...
<script>
    function categorySelect(categories, selected, type) {
        return {
            type: type,
