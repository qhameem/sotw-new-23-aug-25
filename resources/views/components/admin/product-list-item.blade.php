...
<article class="p-4 flex items-center gap-3 md:gap-3 transition relative group">
    ...
    <div class="flex flex-col items-end space-y-2">
        <form action="{{ route('admin.products.updatePromotion', $product) }}" method="POST"
            ...
            <label for="is_promoted_{{ $product->id }}" class="flex items-center cursor-pointer">
                <input type="checkbox" id="is_promoted_{{ $product->id }}" name="is_promoted" value="1"
                    @if($product->is_promoted) checked @endif class="form-checkbox h-4 w-4 text-primary-600">
                <span class="ml-2 text-xs text-gray-700">Promote</span>
            </label>
            <input type="number" name="promoted_position" value="{{ $product->promoted_position }}"
                class="w-16 p-1 border rounded text-xs" placeholder="Pos.">
            <button type="submit"
                class="text-xs bg-blue-500 hover:bg-blue-700 text-white py-1 px-2 rounded">Update</button>
        </form>
