...
@section('title', 'Sign In - Launch Readiness Checker')
...
@section('content')
    <div class="mx-auto max-w-md space-y-8 pt-8">
        ...
        <section
            ...
            class="rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm shadow-slate-200/60"
        >
            ...
            <div x-show="!otpSent" class="space-y-4">
                <a href="{{ route('launch-readiness.auth.google', ['toolSlug' => $toolSlug]) }}"
                   ...
                   class="flex w-full items-center justify-center rounded-2xl border border-slate-300 px-4 py-3 text-sm font-semibold text-slate-900 shadow-sm transition hover:bg-slate-50">
                ...
                <button
                    ...
                    class="flex w-full items-center justify-center rounded-2xl border border-slate-300 bg-white px-4 py-3 text-sm font-semibold tracking-wide text-slate-800 shadow-sm transition hover:bg-slate-50"
                >
            ...
            <div x-show="showEmail && !otpSent" x-transition.opacity.duration.200ms style="display: none;">
                <form method="POST" action="{{ route('launch-readiness.auth.email.send', ['toolSlug' => $toolSlug]) }}" class="space-y-5" @submit="emailSubmitting = true">
                    ...
                    <button
                        ...
                        class="inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-slate-900 px-5 py-3 text-sm font-semibold text-white transition"
                    >
            ...
            <div x-show="otpSent" x-transition.opacity.duration.200ms class="rounded-3xl border border-emerald-200 bg-emerald-50 px-6 py-6 text-center" style="display: none;">
                ...
                <form method="POST" action="{{ route('launch-readiness.auth.email.verify', ['toolSlug' => $toolSlug]) }}" class="mt-5 space-y-4" @submit="otpSubmitting = true">
                    ...
                    <button
                        ...
                        class="inline-flex w-full items-center justify-center gap-2 rounded-2xl border border-slate-900 bg-slate-900 px-5 py-3 text-sm font-semibold text-white transition"
                    >
