...
class ToolAuthController extends Controller
{
    ...
    public function send(Request $request): RedirectResponse
    {
        ...
        if (filled($validated['company_name'] ?? null)) {
            return back()->with('status', 'otp-sent')->with('auth_email', $email);
        }

        $user = ToolUser::query()->where('email', $email)->first();
        $otp = $this->generateOtp();

        ToolAuthMagicLink::query()
            ->where('email', $email)
            ->whereNull('consumed_at')
            ->update(['consumed_at' => now()]);

        ToolAuthMagicLink::create([
        ...
        try {
            Notification::route('mail', $email)->notify(new EmailOtpNotification($otp, self::EXPIRY_MINUTES));
    ...
    public function verify(Request $request): RedirectResponse
    {
        ...
        $magicLink = ToolAuthMagicLink::query()
            ->where('email', $email)
            ->whereNull('consumed_at')
            ->latest('id')
            ->first();
        ...
        if (blank($magicLink->otp_code_hash) || ! hash_equals($magicLink->otp_code_hash, hash('sha256', $otp))) {
            RateLimiter::hit($throttleKey, self::THROTTLE_SECONDS);
    ...
    {
        $user = ToolUser::query()->firstOrCreate(
            ['email' => $magicLink->email],
            [
                'name' => Str::headline(Str::before($magicLink->email, '@')),
    ...
    {
        return 'tool-auth-email-send:'.Str::transliterate($email.'|'.$request->ip());
    }

    private function sendIpThrottleKey(Request $request): string
    {
        return 'tool-auth-email-send-ip:'.Str::transliterate((string) $request->ip());
    }

    private function verifyThrottleKey(string $email, Request $request): string
    {
        return 'tool-auth-email-verify:'.Str::transliterate($email.'|'.$request->ip());
    }
